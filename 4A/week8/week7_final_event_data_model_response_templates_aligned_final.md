...
### Field design notes
- `id`: integer event identifier aligned with Team 4B event API and database schema
...
- `price_line`: include only when price is known
...
|---|---|---|---|
| `id` | Database / API | Integer event identifier from Team 4B database and API | Primary key used for lookup |
...
| `source_site` | Scrape metadata | Normalized source enum (`jkyog` or `radhakrishnatemple`) | Useful for trust/source grouping |
...
- a canonical event schema aligned with Team 4B on event ID type, sponsorship fields, and source_site values
