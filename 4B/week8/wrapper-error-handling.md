...
### Fallback behavior
- Logs contextual error details (amount, currency, payment_intent_id)
- Raises structured application error (`NonRetryableServiceError` or `RuntimeError`)
...
### Fallback behavior
- Logs request context (address, origin, destination, mode)
...
### Fallback behavior
- Logs request context (summary, calendar_id)
- Raises runtime error after retry exhaustion
- Fails fast when circuit breaker is open
...
- While open, requests fail immediately with `CircuitBreakerOpenError`
- Automatically resets after timeout window (30 seconds)
...
Each wrapper exposes a `health_check()` function that reports:
- service name
