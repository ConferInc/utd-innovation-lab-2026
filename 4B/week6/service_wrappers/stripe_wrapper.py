...
class StripeWrapper:
    ...
    def retrieve_payment_intent(self, payment_intent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception(
                "StripeWrapper retrieve_payment_intent failed fast id=%s",
                payment_intent_id,
            )
            raise
        except stripe.InvalidRequestError as exc:
            logger.exception(
                "StripeWrapper invalid retrieve id=%s",
                payment_intent_id,
            )
            raise NonRetryableServiceError(str(exc)) from exc
        except stripe.StripeError as exc:
            logger.exception(
                "StripeWrapper retrieve_payment_intent failed id=%s",
                payment_intent_id,
            )
            raise RuntimeError(f"Stripe error: {str(exc)}") from exc
    ...
    def create_checkout_session(
        ...
        try:
            ...
            logger.exception(
                "StripeWrapper create_checkout_session failed fast amount=%s currency=%s",
                amount,
                currency,
            )
            raise
            ...
            logger.exception(
                "StripeWrapper invalid checkout amount=%s currency=%s",
                amount,
                currency,
            )
            raise NonRetryableServiceError(str(exc)) from exc
            ...
            logger.exception(
                "StripeWrapper create_checkout_session failed amount=%s currency=%s",
                amount,
                currency,
            )
            raise RuntimeError(f"Stripe error: {str(exc)}") from exc
